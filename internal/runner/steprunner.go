...
func (sr *StepRunner) RunStep(ctx context.Context, step workflow.Step, jobEnv map[string]string) (*StepResult, error) {
	...
	cmd := sr.buildCommand(step, jobEnv)

	// Execute in container
	result, err := dockerx.ExecCommand(ctx, sr.cli, sr.containerID, sr.workingDir, cmd)
	if err != nil {
...
func (sr *StepRunner) buildCommand(step workflow.Step, jobEnv map[string]string) []string {
	// Default to bash -e
	shell := "bash"
	if step.Shell != "" {
		shell = step.Shell
	...
	switch shell {
	case "bash", "":
		cmd = []string{"bash", "-e", "-c", step.Run}
	case "sh":
		cmd = []string{"sh", "-e", "-c", step.Run}
	default:
		// Fallback to bash
		cmd = []string{"bash", "-e", "-c", step.Run}
	}
