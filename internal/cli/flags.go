...
func BindRunFlags(cmd *cobra.Command, flags *RunFlags) {
	...
	cmd.Flags().IntVarP(&flags.Parallel, "parallel", "p", 1, "Max parallel jobs")
	cmd.Flags().StringVar(&flags.CRLF, "crlf", "auto", "Line ending handling: auto, lf, crlf")
	...
	cmd.Flags().BoolVar(&flags.Strict, "strict", false, "Strict mode: fail on warnings")
}
